...
async def get_agent_config(agent_name: str) -> Optional[AgentConfig]:
    """Retrieves agent prompt limits and tools from DB."""
    logfire.info(f"Fetching agent config for {agent_name}")
    async with get_db_connection() as conn:
        row = await conn.fetchrow(
            "SELECT agent_name, system_prompt, enabled_tools FROM agent_configs WHERE agent_name = $1",
            agent_name
        )
        if row:
...
async def get_all_agent_configs() -> List[AgentConfig]:
    """Retrieves all agent configurations from DB."""
    logfire.info("Fetching all agent configurations")
    async with get_db_connection() as conn:
        rows = await conn.fetch(
            "SELECT agent_name, system_prompt, enabled_tools FROM agent_configs"
        )
        return [
            AgentConfig(
                agent_name=row['agent_name'],
